...
//#define DEBUG ON //////////////////// Płytka ESP32S2 Dev Module //////////////////
...
void setup() {
	...
	while (ptTryCNT <= 3)
	{
		...
		Serial.print(r);
		Serial.print(F(" Ohms,   "));
		PT00temp = GetPlatinumRTD2(r, 100) - kalibracjaPT100;
		PrintRTDStatus(maxReader.getStatus());
		if (PT00temp > -60 && PT00temp < 70)
		{
			hasMAX = true;
	...
	case Send:
	{
		...
		if (rssi > 4) {
			...
			if (ValidateData(battV, SHTtempRead, SHThumRead, simVoltage))
				sprintf(creq, "api.thingspeak.com/update?api_key=%s&field1=%f&field2=%f&field3=%f&field4=%f&field5=%f&field6=%f&field7=%s&field8=%d", TS_API_KEY, extTemp, PT00temp, BMEhumRead, humidity, DSTempC[1], BMEpressureReadHPA, battV, rdbm);
...
bool SendData(char* creq)
{
	...
	while (!isSendSuccess)
	{
		...
		SIM800.HTTP_GET_start(creq, &statuscode, (uint16_t*)&length);
		...
		if (!isSendSuccess) {
			...
			SIM800.HTTP_GET_end();
			SIM800.TCPclose();
			if (rtryCount > 2 && rtryCount <= 5)
			{
				ToggleGSModule(false);
				InitGSM();
				delay(2000);
			}
			if (rtryCount == 6)
