...
void setup() {
	...
	while (ptTryCNT <= 3)
	{
		...
		PT100temp = temperature(100, RTD_TYPE_PT100_R430);
...
void loop() {
	...
	case CollecData:
	{
		...
		dtostrf(busVoltage, 5, 3, battV);

		simVoltage = (float)simBat / 1000;
		dtostrf(simVoltage, 5, 3, simV);
	...
	case SendMainData:
	{
		...
		if (ValidateData(battV, SHTtempRead, SHThumRead, simVoltage))
			sprintf(creq, "api.thingspeak.com/update?api_key=%s&field1=%f&field2=%f&field3=%f&field4=%f&field5=%f&field6=%f&field7=%s&field8=%d", TS_API_KEY, SHTtempRead, PT100temp, DSTempC[0], SHThumRead, DSTempC[1], BMEpressureReadHPA, battV, rdbm);
		...
		if (!SendData(creq))
		{
			...
			Serial.println(F("Sending first channel Success..."));
	...
	case SendSubData:
	{
		Serial.println(F("State:SendSubData"));
		char creq[230];
		...
		//if main send was ok then we dont need to care for the second packet, just go sleep.
		if (mainSendSuccess) {
			_state = Sleep;
		}else		
		{
...
void GetPT100Readings()
{
	...
	Serial.print(PT100temp + kalibracjaPT100);
...
void clearFault(void) {
	uint8_t t = maxReader.readByte(REG_CFG);
	t &= ~0x2C;
	t |= MAX31865_CONFIG_FAULTSTAT;
	maxReader.writeByte(REG_CFG, t);
}
...
uint16_t readRTD(void) {
	...
	uint16_t rtd = maxReader.readWord(REG_RESISTANCE);
...
void GetBMEReadings()
{
	...
	BMEpressureReadHPA = bme280.readPressure() / 100.0F + 25;
